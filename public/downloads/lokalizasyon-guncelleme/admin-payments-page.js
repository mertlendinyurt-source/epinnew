...
function IbanToggle() {
  const [ibanEnabled, setIbanEnabled] = useState(true);
  const [ibanLoading, setIbanLoading] = useState(true);
  const [ibanToggling, setIbanToggling] = useState(false);
  ...
  const loadIbanSettings = async () => {
    try {
      ...
      const res = await fetch('/api/admin/settings/iban', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) {
        setIbanEnabled(data.data.isEnabled !== false);
      }
    } catch (err) {
      console.error('IBAN settings load error:', err);
    } finally {
  ...
  const toggleIban = async () => {
    setIbanToggling(true);
    try {
      const token = localStorage.getItem('userToken') || localStorage.getItem('adminToken');
      const res = await fetch('/api/admin/settings/iban', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ isEnabled: !ibanEnabled })
      });
      const data = await res.json();
      if (data.success) {
        setIbanEnabled(!ibanEnabled);
      }
    } catch (err) {
      console.error('IBAN toggle error:', err);
    } finally {
      setIbanToggling(false);
    }
  };
    ...
    <div className="flex items-center justify-between">
      <div>
        <h3 className="text-sm font-semibold text-gray-300 mb-1 flex items-center gap-2">
          🏦 Havale / EFT (IBAN) Ödemesi
        </h3>
        <p className="text-sm text-gray-400">
          {ibanEnabled ? 'IBAN ile ödeme aktif - müşteriler havale yapabilir' : 'IBAN ile ödeme kapalı - müşteriler göremez'}
        </p>
        <div className="flex items-center gap-2 mt-2">
          <span className={`w-2.5 h-2.5 rounded-full ${ibanEnabled ? 'bg-green-500' : 'bg-red-500'}`}></span>
          <span className={`text-xs font-medium ${ibanEnabled ? 'text-green-400' : 'text-red-400'}`}>
            {ibanEnabled ? 'Aktif' : 'Pasif'}
          </span>
        </div>
      </div>
      <button
        onClick={toggleIban}
        disabled={ibanToggling || ibanLoading}
        className={`relative w-14 h-7 rounded-full transition-colors ${
          ibanEnabled ? 'bg-green-500' : 'bg-gray-600'
        } ${ibanToggling ? 'opacity-50' : ''}`}
      >
        <span className={`absolute top-0.5 w-6 h-6 bg-white rounded-full shadow transition-transform ${
          ibanEnabled ? 'translate-x-7' : 'translate-x-0.5'
        }`}></span>
      </button>
    </div>
  );
