...
async function createShopinextPayment(db, order, user, product) {
  ...
  try {
    ...
    const paymentPayload = {
      ...
      identity_number: '11111111111', // Dijital ürünler için varsayılan TC
      company: '',
      tax_office: '',
      tax_number: '',
...
async function sendPaymentFailedSms(db, order, user) {
  ...
  const customerName = user.firstName || user.name || 'Müşteri';
  const message = `${customerName} Merhaba, odemeniz basarisiz oldu. Garanti Bankasi kartlari su anda bakimdadir. Lutfen baska bir banka karti ile tekrar deneyin. pinly.com.tr - PINLY`;
  ...
  const content = {
    ...
    body: `
      ...
      <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:16px;margin:20px 0;">
        ...
        <p style="margin:0;color:#856404;font-size:14px;">Garanti Bankası ödeme sistemleri şu anda bakımdadır. Lütfen <strong>farklı bir banka kartı</strong> ile tekrar deneyin veya <strong>IBAN (Havale/EFT)</strong> seçeneği ile ödeme yapın.</p>
      </div>

      <div style="background:#d4edda;border:1px solid #28a745;border-radius:8px;padding:16px;margin:20px 0;">
        <p style="margin:0 0 8px 0;font-weight:bold;color:#155724;">💳 Ne Yapabilirsiniz?</p>
        <ul style="margin:0;padding-left:20px;color:#155724;font-size:14px;">
          <li style="margin-bottom:6px;">Farklı bir banka kartı ile tekrar ödeme yapabilirsiniz</li>
          <li><strong>IBAN (Havale/EFT)</strong> seçeneği ile anında ödeme yapabilirsiniz</li>
        </ul>
      </div>
    ...
    if (pathname === '/api/payment-methods') {
      ...
      return NextResponse.json({
        ...
        data: {
          ...
          odesin: {
            available: !!(process.env.ODESIN_API_KEY),
            name: 'Otomatik Havale (Ödesin)'
          }
        }
      });
    }
...
export async function POST(request) {
  ...
  try {
    ...
    if (pathname === '/api/orders') {
      ...
      // ============================================
      // 🏦 ÖDESİN HOSTED HAVALE FLOW (Otomatik IBAN)
      // ============================================
      if (paymentMethod === 'odesin') {
        const ODESIN_API_KEY = process.env.ODESIN_API_KEY || '';
        
        if (!ODESIN_API_KEY) {
          return NextResponse.json(
            { success: false, error: 'Ödesin ödeme sistemi yapılandırılmamış.' },
            { status: 503 }
          );
        }
        ...
        // Create Ödesin payment session
        try {
          const odesinRes = await fetch('https://www.odesin.com/api/payment/create-session', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            ...
            body: JSON.stringify({
              ...
              description: `PINLY-${order.id}`,
              callback_url: `${BASE_URL}/api/payment/odesin/callback`,
              return_url: `${BASE_URL}/payment/success?orderId=${order.id}`,
              currency: 'TRY',
              metadata: { orderId: order.id, productId, playerId }
            })
          });

          const odesinData = await odesinRes.json();
          
          if (!odesinData.checkout_url) {
            console.error('Ödesin session error:', odesinData);
            await db.collection('orders').updateOne({ id: order.id }, { $set: { status: 'failed', failReason: 'Ödesin oturum oluşturulamadı' } });
            return NextResponse.json({ success: false, error: 'Ödeme oturumu oluşturulamadı' }, { status: 500 });
          }
          ...
          await db.collection('orders').updateOne(
            ...
            { $set: { 
              odesinSession: {
                sessionId: odesinData.session_id,
                checkoutUrl: odesinData.checkout_url,
                referenceCode: odesinData.reference_code,
                expiresAt: odesinData.expires_at
              },
              updatedAt: new Date()
            }}
          );
          ...
          console.error('Ödesin API error:', odesinErr);
          await db.collection('orders').updateOne({ id: order.id }, { $set: { status: 'failed', failReason: 'Ödesin API hatası' } });
          return NextResponse.json({ success: false, error: 'Ödeme sistemi bağlantı hatası' }, { status: 500 });
        }
      }
    ...
    // ============================================
    // 🏦 ÖDESİN WEBHOOK CALLBACK HANDLER
    // ============================================
    if (pathname === '/api/payment/odesin/callback') {
      console.log('Ödesin webhook received:', JSON.stringify(body));
      
      const { event, session_id, reference_code, amount, currency, status, transaction_id, timestamp } = body;
      
      // Find order by session_id
      const order = await db.collection('orders').findOne({ 'odesinSession.sessionId': session_id });
      
      if (!order) {
        console.error('Ödesin webhook: order not found for session:', session_id);
        return NextResponse.json({ received: true, error: 'order_not_found' }, { status: 200 });
      }

      // Idempotent check - don't process twice
      if (order.status === 'paid' && event === 'payment.approved') {
        console.log('Ödesin webhook: order already paid:', order.id);
        return NextResponse.json({ received: true, message: 'already_paid' }, { status: 200 });
      }

      if (event === 'payment.approved' && status === 'paid') {
        // Verify with Ödesin session-status API
        const ODESIN_API_KEY = process.env.ODESIN_API_KEY || '';
        let verified = false;
        try {
          const verifyRes = await fetch(`https://www.odesin.com/api/payment/session-status?session_id=${session_id}`, {
            headers: { 'x-api-key': ODESIN_API_KEY }
          });
          const verifyData = await verifyRes.json();
          if (verifyData.status === 'paid') {
            verified = true;
          }
          console.log('Ödesin verify result:', verifyData);
        } catch (verifyErr) {
          console.error('Ödesin verify error:', verifyErr);
        }

        if (!verified) {
          console.error('Ödesin webhook: verification failed for session:', session_id);
          return NextResponse.json({ received: true, error: 'verification_failed' }, { status: 200 });
        }
        ...
        await db.collection('payments').insertOne({
          id: uuidv4(), orderId: order.id, provider: 'odesin', providerTxnId: transaction_id || null,
          status: 'paid', amount: order.amount, currency: order.currency || 'TRY',
          source: 'webhook', rawPayload: body, verifiedAt: new Date(), createdAt: new Date()
        });
        ...
        if (orderUser && orderUser.phone) {
          sendPaymentSuccessSms(db, order, orderUser, product?.title || 'Sipariş').catch(err => console.error('SMS err:', err));
        }
        if (orderUser && product) {
          sendPaymentSuccessEmail(db, order, orderUser, product).catch(err => console.error('Email err:', err));
        }

        // Check verification requirement (high value orders)
        const orderAmount = order.amount || 0;
        if (orderAmount >= 3000) {
          await db.collection('orders').updateOne({ id: order.id }, { $set: {
            verification: { required: true, status: 'pending', identityPhoto: null, paymentReceipt: null, submittedAt: null },
            delivery: { status: 'verification_required', message: 'Yüksek tutarlı sipariş - Doğrulama gerekli', items: [] }
          }});
          if (orderUser && product) sendVerificationRequiredEmail(db, order, orderUser, product).catch(err => console.error('Verif email err:', err));
        } else {
          // Risk check
          const riskResult = await calculateOrderRisk(db, order, orderUser, request);
          await db.collection('orders').updateOne({ id: order.id }, { $set: { risk: riskResult } });
          ...
          try {
            if (product) {
              ...
              if (assignedStock && assignedStock.value) {
                await db.collection('orders').updateOne({ id: order.id }, { $set: { delivery: { status: 'delivered', items: [assignedStock.value], stockId: assignedStock.id || assignedStock._id, assignedAt: new Date() } } });
                if (orderUser) sendDeliveredEmail(db, order, orderUser, product, [assignedStock.value]).catch(err => console.error('Del email err:', err));
              } else {
                // Try DijiPin
                const dijipinSettings = await db.collection('settings').findOne({ type: 'dijipin' });
                if (dijipinSettings?.isEnabled && product.dijipinEnabled) {
                  try {
                    ...
                    if (dijipinResult.success && dijipinResult.pin) {
                      await db.collection('orders').updateOne({ id: order.id }, { $set: { delivery: { status: 'delivered', items: [dijipinResult.pin], method: 'dijipin', deliveredAt: new Date() } } });
                    } else {
                      await db.collection('orders').updateOne({ id: order.id }, { $set: { delivery: { status: 'pending', message: 'Stok bekleniyor', items: [] } } });
                    }
                  } catch (e) {
                    await db.collection('orders').updateOne({ id: order.id }, { $set: { delivery: { status: 'pending', message: 'Stok bekleniyor', items: [] } } });
                  }
          ...
          } catch (stockErr) { console.error('Ödesin stock error:', stockErr); }
        }

        console.log('Ödesin: Order paid and processed:', order.id);
      } else if (event === 'payment.rejected') {
        await db.collection('orders').updateOne(
          { id: order.id },
          { $set: { status: 'failed', paymentProvider: 'odesin', failReason: 'Havale reddedildi', updatedAt: new Date() } }
        );
        
        const orderUser = await db.collection('users').findOne({ id: order.userId });
        if (orderUser) {
          sendPaymentFailedEmail(db, order, orderUser).catch(err => console.error('Fail email err:', err));
          if (orderUser.phone) sendPaymentFailedSms(db, order, orderUser).catch(err => console.error('Fail SMS err:', err));
        }
      }
    ...
    // ============================================
    // 💳 PAYYEEN WEBHOOK CALLBACK HANDLER
    // ============================================
    if (pathname === '/api/payment/payyeen/callback') {
      console.log('Payyeen webhook received:', JSON.stringify(body));
      
      const { event, transaction_id, amount, currency, status, description, created_at } = body;
      
      // Extract orderId from description (PINLY-{orderId})
      let orderId = null;
      if (description && description.startsWith('PINLY-')) {
        orderId = description.replace('PINLY-', '');
      }
      
      // Fallback: find via payment_requests collection
      if (!orderId) {
        const paymentRequest = await db.collection('payment_requests').findOne({
          provider: 'payyeen',
          description: description
        });
        if (paymentRequest) orderId = paymentRequest.orderId;
      }
      
      if (!orderId) {
        console.error('Payyeen callback: Cannot determine orderId from description:', description);
        return new Response('OK', { status: 200 });
      }
      ...
      const order = await db.collection('orders').findOne({ id: orderId });
      if (!order) {
        console.error(`Payyeen callback: Order ${orderId} not found`);
        return new Response('OK', { status: 200 });
      }
      
      // 2. Check if order is already PAID (idempotency protection)
      if (order.status === 'paid') {
        console.log(`Payyeen callback: Order ${orderId} already PAID. Ignoring duplicate callback.`);
        return new Response('OK', { status: 200 });
      }
      
      // 3. Map Payyeen event/status to application status
      ...
      if (event === 'payment.success' || status === 'success') {
        ...
        console.log(`Payyeen callback: Order ${orderId} still pending`);
        return new Response('OK', { status: 200 });
      } else {
        newStatus = 'failed';
      }
      
      // 4. Enforce immutable status transitions
      if (order.status === 'failed' && newStatus === 'paid') {
        console.error(`Payyeen callback: Cannot change order ${orderId} from FAILED to PAID`);
        return new Response('OK', { status: 200 });
      }
      ...
      await db.collection('payment_requests').updateOne(
        ...
        { $set: { status: newStatus, transactionId: transaction_id, updatedAt: new Date() } }
      );
      
      // 7. PROCESS PAID ORDERS (Stock assignment etc - same as Shopier/Shopinext)
      if (newStatus === 'paid') {
        const orderUser = await db.collection('users').findOne({ id: order.userId });
        const product = await db.collection('products').findOne({ id: order.productId });
        ...
        if (orderUser && orderUser.phone) {
          const itemTitle = product?.title || 'Sipariş';
          console.log('Payyeen: Sending payment SMS to', orderUser.phone);
          sendPaymentSuccessSms(db, order, orderUser, itemTitle).catch(err =>
            console.error('Payyeen payment SMS failed:', err)
          );
        }
        
        // Check if this is an account order
        if (order.type === 'account' && order.accountId) {
          // Account order - deliver credentials
          const account = await db.collection('accounts').findOne({ id: order.accountId });
          if (account && account.credentials) {
            await db.collection('orders').updateOne(
              { id: orderId },
          ...
          try {
            ...
            console.error(`Payyeen stock error for order ${orderId}:`, stockError);
          }
        }
      ...
      console.log(`Payyeen callback: Order ${orderId} status updated to ${newStatus}`);
    ...
    if (pathname === '/api/support/tickets') {
      ...
      const autoReply = {
        ...
        isAutoReply: true,
        createdAt: new Date(Date.now() + 1000) // 1 saniye sonra (sıralama için)
      };
      await db.collection('ticket_messages').insertOne(autoReply);

      // Ticket durumunu güncelle - admin yanıt verdi
      await db.collection('tickets').updateOne(
        { id: ticket.id },
