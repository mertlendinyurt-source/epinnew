...
export async function DELETE(request) {
  ...
  try {
    ...
    // ============================================
    // 🔐 VERIFICATION ENDPOINTS (High-Value Orders >= 3000 TL)
    // ============================================
    ...
    if (method === 'GET' && pathname.match(/^\/api\/account\/orders\/([^\/]+)\/verification$/)) {
      const user = verifyToken(request);
      if (!user || user.type !== 'user') {
        return NextResponse.json({ success: false, error: 'Giriş gerekli' }, { status: 401 });
      }
    ...
    if (method === 'GET' && pathname === '/api/admin/orders/pending-verification') {
      const adminUser = verifyAdminToken(request);
      if (!adminUser) {
        return NextResponse.json({ success: false, error: 'Yetkisiz erişim' }, { status: 401 });
      }
    ...
    if (method === 'PUT' && pathname.match(/^\/api\/admin\/orders\/([^\/]+)\/verify$/)) {
      const adminUser = verifyAdminToken(request);
      if (!adminUser) {
        return NextResponse.json({ success: false, error: 'Yetkisiz erişim' }, { status: 401 });
      }
